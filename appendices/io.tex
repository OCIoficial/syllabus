...
A continuación se detallan las formas recomendadas de hacerlo en {\new \java, \cpp y \python}.
...
\paragraph{\python.} {\new
  La forma recomendada en \python para leer de la entrada estándar es
  utilizando la función \code{input()}.
  Para escribir a la salida estándar se debe utilizar la función \code{print()}.
  
  Es importante tener en cuenta que \code{input()} lee líneas completas como
  cadenas de texto, por lo que puede ser necesario utilizar métodos como
  \code{split()} para separar múltiples valores y funciones como \code{int()}
  o \code{float()} para convertir los valores al tipo de dato apropiado.
}
